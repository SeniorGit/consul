...
export default function PatientApp() {
  ...
  const renderMessagePart = useCallback((messagePart: MixedTextTypedElement) => {
    if (messagePart.type === "text") return messagePart.content.text;
    if (messagePart.type === "plainLink") return <a href={messagePart.content.link}>{messagePart.content.link}</a>;
    if (messagePart.type === "textLink") return <a href={messagePart.content.link}>{messagePart.content.text}</a>;
    if (messagePart.type === "mention") return <a href={`https://pubnub.com/${messagePart.content.id}`}>{messagePart.content.name}</a>;
    return "";
  }, []);
  ...
  if (!chat || !channel) {
    return <p className="text-white">Initializing Chat... (If this persists, check your keys or network connection)</p>;
  }
  ...
  return (
    <section className="min-h-screen h-screen flex flex-col lg:flex-row p-5 gap-7 w-full pt-[60px]" style={{ background: "linear-gradient(to top, #ffffff 60%, #003A7C 100%)" }}>
      ...
      <main className={`${isChatOpen ? "block" : "hidden"} w-full sm:w-full lg:flex lg:flex-1 flex-col bg-gray-100 rounded-lg shadow-lg h-[750px]`}>
        <button onClick={() => setIsChatOpen(false)} className="sm:block lg:hidden text-white p-2 mb-2 bg-blue-500 rounded-lg">
          Back
        </button>
              ...
              messages.map((message) => {
                ...
                return (
                  <li key={message.timetoken} className="flex items-start mb-4">
                    <aside className="flex-shrink-0 w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-white mr-4" style={{ background: String(user?.custom?.avatar) }}>
                    ...
                    <article>
                      <h3 className="text-gray-800">
                        ...
                        <time className="text-gray-500 text-sm ml-2">
                          {TimetokenUtils.timetokenToDate(message.timetoken).toLocaleTimeString([], { timeStyle: "short" })}
                        </time>
                      </h3>
