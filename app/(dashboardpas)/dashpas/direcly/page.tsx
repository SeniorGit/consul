...
export default function Directly() {
  ...
  const handleApply = () => {
    if (selectedTime.length === 2) {
      const formattedTimeRange = `${selectedTime[0]} - ${selectedTime[1]}`;
      setDisplayTime(
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="ml-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {["Stress", "Trauma", "Anxiety"].map((item, i) => (
                      <span key={i} className="bg-[#4C82CB] text-white px-3 py-1 rounded-full text-xs shadow-lg">
                        {item}
                      </span>
                    ))}
                  </div>
                  <div className="flex items-center mt-3 space-x-3">
                    ...
                    <span className="bg-[#4C82CB] flex items-center px-3 py-1 rounded-full text-xs shadow-lg">
                      <Image src="/icons/suitcase.png" alt="Experience" width={16} height={16} className="mr-1" />
                      6 Years
                    </span>
                  </div>
                </div>
            ...
            <div className="p-4 bg-white shadow-lg rounded-lg">
              ...
              <div className="grid grid-cols-7 gap-2 text-center mb-4">
                {dates.map((date) => (
                  <button
                    ...
                    className={`p-2 rounded-lg ${date.toDateString() === selectedDate.toDateString() ? "bg-blue-200" : ""}`}
                  >
              ...
              <div className="flex justify-between items-center border-t pl-5 bg-blue-100 rounded-full border-[#4C82CB] border-[1px]">
                <span className="font-semibold text-gray-800">{displayTime || "Select a time"}</span>
                <button onClick={handleApply} className="bg-[#27374D] text-white px-4 py-2 rounded-full hover:bg-[#1f2c3d] transition">
                  Apply
                </button>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4">
              ...
              <div className="bg-[#DBE5F2] p-4 rounded-xl flex items-start space-x-4">
                <img src="/images/cewe.png" alt="Psychologist" className="w-12 h-12 rounded-full" />
                <div>
                  <h3 className="font-bold text-[#27374D]">Isey S.Psi, M.Psi. Psychologists</h3>
                  ...
                  <Link href="/dashpas/chat">
                    <button className="mt-2 bg-[#4D9CE1] text-white px-6 py-2 rounded-lg">Chat Now!</button>
                  </Link>
                </div>
              </div>
